		...
		*/
		...
		if($queries!=null) {
			...
			foreach($queries as $query){
				...
				if(isset($query["clicks"])){
					...
					foreach($clicks as $click){
						...
						$sqlClick = "INSERT INTO ".$CONFIG['db_tb_logclick']." ".
								...
								"queryid,page,`index`,title,url,button) VALUES ". $sql;
				...
				$SearchGoal = mysql_real_escape_string($_POST["q${qNo}_SearchGoal"]);
				$IsTryRemember = strtolower(trim(mysql_real_escape_string($_POST["q${qNo}_IsTryRemember"])))=="yes"?true:false;
				$SearchTime = mysql_real_escape_string($_POST["q${qNo}_SearchTime"]);
				$IsSuccessful = strtolower(trim(mysql_real_escape_string($_POST["q${qNo}_IsSuccessful"])))=="yes"?true:false;
				...
				$sqlQuery = "INSERT INTO ".$CONFIG['db_tb_questionaire']." ".
						"(queryid,SearchGoal,IsTryRemember,SearchTime,IsSuccessful) VALUES ".$sql;
				$result = mysql_query($sqlQuery);
				if(!$result){
